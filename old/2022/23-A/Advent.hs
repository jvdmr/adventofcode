...
step (mpfs, elves) = (tail mpfs, sortBy compareYX $ recenter $ move $ propose mpfs elves)

addMPs :: Elves -> ([MoveProposer], Elves)
addMPs elves = (moveProposers, elves)

countEmptyTiles :: Elves -> Int
countEmptyTiles elves = (mx * my) - length elves
  where (mx, my) = bounds elves

main = do
  cnt <- getContents
  print $ sum $ map countEmptyTiles $ map (ftrace drawElves) $ map snd $ take 11 $ iterate step $ addMPs $ findElves $ lines cnt
  print $ countEmptyTiles $ snd $ last $ take 11 $ iterate step $ addMPs $ findElves $ lines cnt
