...
end g = map fst $ filter ((== 'a') . snd) $ zip (allcoords g) (concat g)

start :: Graph Char -> Coord
start g = fst $ head $ filter ((== 'E') . snd) $ zip (allcoords g) (concat g)
...
allowedRange c = [prv c..'z']

climb :: Graph Char -> Distance
climb g = head $ sort $ map (val dg) $ end g
  where g' = unmark g
        distance _ _ _ = 1
