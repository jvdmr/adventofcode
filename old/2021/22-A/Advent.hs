...
switchCubes cubes ins@(Off _ _ _) = concat $ map (filter notEmpty . cutCubeRange ins) cubes

filterSteps' (xa, xb) (ya, yb) (za, zb) = xb < -50 || xa > 50 || yb < -50 || ya > 50 || zb < -50 || za > 50

filterSteps (On xs ys zs) = filterSteps' xs ys zs
filterSteps (Off xs ys zs) = filterSteps' xs ys zs

main = do
  cnt <- getContents
--   print $ foldl (+) 0 $ map countCubes $ foldl (switchCubes . ftrace (map countCubes) . idtrace) [] $ map idtrace $ filter (not . filterSteps) $ map parseStep $ lines cnt
  print $ foldl (+) 0 $ map countCubes $ foldl switchCubes [] $ filter (not . filterSteps) $ map parseStep $ lines cnt
