...
fight ((total, PTurn, boss, player, effects):rest) | mna player < 53 = fight rest
  ...
  where nextFights = (map nextFight $ filter castable spells) ++ rest
        nextFight s = (total + cost s, BTurn, boss { hp = max 0 $ (hp boss) - (totaldmg effects) }, player { hp = max 0 $ (hp player) + (totalheal effects) - 1, mna = max 0 $ (mna player) - (cost s) + (totalmana effects) }, sort $ cast s:nextEffects)
