...
applyRules ruleMap seen ((s, n):rest) | elem s (keys ruleMap) = applyRules ruleMap (s:seen) $ (ruleMap ! s, n + 1):rest
                                      | otherwise = applyRules ruleMap (s:seen) $ sortBy fstLength $ (map (\rs -> idtrace (rs, n + 1)) $ filter (not . flip elem (seen ++ map fst rest)) $ concat $ map applyRule $ uniq $ sort $ filter (flip elem safeKeys) $ sublists s) ++ rest
