...
--                                 where noblizzard xy = notElem xy $ ftrace ((++) (show n' ++ " - " ++ show xy ++ " - ") . show) $ step (bounds, rbf xy) n'
                                where noblizzard xy = notElem xy $ step (bounds, rbf xy) n'
                                      n' = n + 1
                                      next = filter (flip notElem rst) $ zip [n', n'..] $ filter noblizzard $ nb e

steps :: Map -> Int
steps (bounds, blizzards) = 1 + (fst $ idtrace $ f [(go stay (back + 2), stay)])
  where end = bounds - (1, 1)
        f = steps' bounds rbf nb end f
        f' = steps' bounds rbf nb stay f'
        nb = neighbors bounds
        rbf = relevantBlizzards bounds blizzards
        (there, _) = idtrace $ f [(1, stay)]
        go xy t | notElem xy $ step (bounds, rbf xy) t = t
                | otherwise = go xy (t + 1)
        (back, _) = idtrace $ f' [(go end (there + n), end) | n <- [2..20]]
