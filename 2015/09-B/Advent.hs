...
doTsp g visited n | nextNodes == [] = [[]]
                  | otherwise = flatten $ map (\next -> map (next:) (doTsp g (n:visited) $ getDestination next)) nextNodes
                  where nextNodes = filter (not . flip elem visited . getDestination) $ (g ! n)

tsp :: Graph -> [[Distance]]
tsp g = flatten $ map (doTsp g []) ks
  where ks = keys g

main = do
  cnt <- getContents
  print $ last $ sort $ map (foldl (+) 0 . map getDistance) $ tsp $ createGraph M.empty $ idtrace $ map parseString $ lines cnt
