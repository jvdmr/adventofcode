...
instance Show BurrowSpace where
  show (Space a) = show a
  show (Door _ as) = flatten $ ".":map show as
...
instance Show Burrow where
  show (Burrow burrow) = flatten $ map (++ "\n") $ take 5 $ transpose $ map ((++ "####") . show) burrow
...
parseBurrow (_:s:as) = Burrow $ zipWith makeDoors rooms $ map (\'.' -> Space Empty) $ init $ tail s
  where rooms = makeRooms $ tail $ occupants $ map (++ "###") $ take 4 as
        occupants ([]:_) = []
...
possibleMoves burrow@(Burrow bs) | organizedBurrow burrow = []
                                 | otherwise = flatten $ map (possibleMovesFrom burrow) $ take (length bs) [0..]
