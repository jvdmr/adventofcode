...
resolvePart (stack, model) (i, part) = (stack', model')
  where (Div (Var 'z') (Val a)) = part !! 3
        ...
        model' = if a == 1 then model else insert i' (1 + diff') $ insert j' 1 model
...
findInput algo = listToInt $ map snd $ toList $ snd $ foldl resolvePart ([], empty) $ zip [0..] $ ftrace (flatten . map ((++"\n") . show)) $ tail $ splitOn [Inp (Var 'w')] algo
