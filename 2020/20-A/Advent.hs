...
instance Show TileFile where
    show tile = show $ foldl ($$) (text "") $ map text $ ("Tile " ++ (show $ tileId tile) ++ ":"):(chunksOf (sizeX tile) $ showTileData tile)
...
showTileData tile = map (loc tile) coords
  where coords = flatten $ mapEach (map (flip (,)) [0..my]) [0..mx]
        (mx, my) = addCoords (-1, -1) $ orientedSize tile
...
showImage image = foldl ($$) (text "") $ map text $ join [""] $ map (map (join " ") . mapEach (map lineN [0..tmy])) $ chunksOf es image
  where es = edgeSize image
        (_, tmy) = addCoords (-1, -1) $ orientedSize $ head image

findNextTiles tile dir tiles = flatten $ map (mutateTileToMatch tile dir) $ filter ((/=) tile) tiles

findImageAlgorithm :: Int -> Int -> Image -> [[TileFile]] -> [TileFile] -> Image
findImageAlgorithm n es [] [] tiles = findImageAlgorithm (n+1) es [] [flatten $ map mutations tiles] tiles
...
cornersProduct image = foldl (*) 1 $ map (tileId . locTile image es) $ [(0, 0), (0, my), (mx, 0), (mx, my)]
  where es = edgeSize image
        (mx, my) = addCoords (-1, -1) (es, es)
...
--   print $ showImage $ parseTileFile newTileFile $ lines cnt
  ...
  print $ cornersProduct $ findImage $ parseTileFile newTileFile $ lines cnt
