...
cubeSurroundings space pos = map (loc space . add pos) diffs
  where diffs = filter (/= origin) $ flatten $ mapEach (flatten $ mapEach (flatten $ mapEach (map coords distances) distances) distances) distances
        distances = [-1, 0, 1]
...
createSpace plane = [[init $ flatten $ map ((:[negativeSpace]) . init . flatten . map (:['.'])) plane]]
  where negativeSpace = listN (2 * xsize - 1) '.'
        xsize = (length $ head plane)

flattenSpace = flatten . flatten . flatten
